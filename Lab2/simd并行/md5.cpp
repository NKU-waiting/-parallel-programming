...
void MD5Hash(const string inputs[4], bit32 states[4][4]) 
{
    ...
    // 初始化四个独立的状态向量
    states_neon[0] = vdupq_n_u32(0x67452301);
    states_neon[1] = vdupq_n_u32(0xefcdab89);
    states_neon[2] = vdupq_n_u32(0x98badcfe);
    states_neon[3] = vdupq_n_u32(0x10325476);

    // 预处理四个输入消息
    for (int i = 0; i < 4; ++i) {
        paddedMessages[i] = StringProcess(inputs[i], &messageLengths[i]);
        assert(messageLengths[i] == messageLengths[0]); // 需要保证长度一致
    }
    ...
    for (int blk = 0; blk < n_blocks; ++blk) {
        ...
        for (int i = 0; i < 16; ++i) {
            uint32_t words[4];
            for (int j = 0; j < 4; ++j) {
                const Byte* block = paddedMessages[j] + blk * 64;
                words[j] = (block[4*i])       | (block[4*i+1] << 8) 
    ...
    // 将向量状态转换回标量数组
    for (int i = 0; i < 4; ++i) {
        uint32_t state_values[4];
        vst1q_u32(state_values, states_neon[i]);
        
        for (int j = 0; j < 4; ++j) {
            // 字节序转换
            uint32_t value = state_values[j];
