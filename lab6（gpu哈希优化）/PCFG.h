...
public:
    ...
    bool operator<(const PT& other) const {
        return this->prob > other.prob; // Higher prob means "smaller"
    }
...
{
public:
    // from std::vector<PT> to std::multiset<PT>
    std::multiset<PT> priority; // <--- The core change!
    ...
    // --- 修改/新增的函数声明 ---
    void UpdateQueueFromTop(); // 代替PopNext的部分功能
    void Generate_CPU(PT pt); // 原Generate函数，用于CPU
    void DispatchToGpu(const vector<PT>& pt_batch); // 新增，分发任务到GPU

    // PopNext被main函数中的逻辑替代，可以移除或注释掉
    // void PopNext();
