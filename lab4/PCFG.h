...
public:
    ...
    // === 新增代码: 为 std::priority_queue 提供比较逻辑 ===
    // 我们希望概率大的 PT 优先级高，所以当 pt1.prob < pt2.prob 时，定义 pt1 < pt2
...
public:
    ...
    int Find(const segment& seg)
    {
        if (seg.type == 1) return FindLetter(seg);
        if (seg.type == 2) return FindDigit(seg);
        if (seg.type == 3) return FindSymbol(seg);
        return -1;
    };
};
...
public:
    ...
    void BatchGenerate(int batch_size);
};